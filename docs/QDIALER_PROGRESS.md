...
- VICIDIAL admin access requires `www/vicidial/project_auth_entries.txt` to be writable by Apache/PHP; use `scripts/vps/fix-vicidial-runtime-permissions.sh` after fresh VPS deploys so the runtime auth log is writable and ignored by deployed worktree status.
