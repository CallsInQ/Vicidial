...
if ($ACTION=="RedirectFromPark")
	...
	if ($ACTION=="RedirectToParkXfer")
	{
		...
		else
		{
			...
			$stmt = "INSERT INTO park_log SET uniqueid='$uniqueid',status='PARKEDXFER',channel='$channel',channel_group='$campaign',server_ip='$server_ip',parked_time='$NOW_TIME',parked_sec=0,extension='$CalLCID',user='$user',lead_id='$lead_id';";
			if ($format=='debug') {echo "\n<!-- $stmt -->";}
			$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02098',$user,$server_ip,$session_name,$one_mysql_log);}
	...
	if ($ACTION=="RedirectFromParkXfer")
	{
		...
		else
		{
			...
			if ($VAC_pl_ct > 0)
			{
				...
				$stmt = "UPDATE park_log SET status='GRABBEDXFR',grab_time='$NOW_TIME',parked_sec='$parked_sec' where uniqueid='$uniqueid' and server_ip='$server_ip' and extension='$CalLCID' order by parked_time desc limit 1;";
				if ($format=='debug') {echo "\n<!-- $stmt -->";}
				$rslt=mysql_query($stmt, $link);
				if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02103',$user,$server_ip,$session_name,$one_mysql_log);}
